...
const builtInContextValidation = require('wa-built-in-context');
...
Handler.prototype.setBuiltInContextProperty = function(context, path, value) {
    const validationResult = builtInContextValidation.validateBuiltinContext(object);
    if(validationResult.valid) {
        const result = builtInContextValidation.setProperty(context.builtIn, path, value);
        if(result.valid) {
            console.info('Successfully set built-in context property');
            return result
        } else {
            console.error('Error setting built-in context property');
            return result;
        }
    } else {
        console.error('Error setting built-in context property');
...
Handler.prototype.getBuiltInContextProperty = function(context, path) {
    const result = builtInContextValidation.getProperty(context.builtIn, path);
    if(result.valid) {
        return result.value;
    } else {
        console.error('Error getting built-in context: ' + result.msg);
        return undefined;
    }
};
 ...
 * @param builtInContext - user altered built-in context
 * @returns {boolean}
 */
Handler.prototype.validateBuiltInContext = function(builtInContext) {
