...
Handler.prototype.evaluateRequest = function (request, evaluationResponse, context, WACreds, callback) {
    ...
    if(WACreds) {
        this.createWAEngine(request, response, context, WACreds, callback)
            .then(({WAEngine, conversation}) => {
                getIntentity(self, request, WAEngine, (err, intentResult, output) => {
...
Handler.prototype.createConversationPayload = function(request, WACreds, context) {
    return {
        workspace_id: WACreds.workspaceID,
 ...
 * @param callback
 */
Handler.prototype.createWAEngine = function(request, response, context, WACreds, callback) {
    new Promise((resolve, reject) => {
        let tenantEngine = new WAEngine('wcs');
        tenantEngine.init(request.WACreds)
            .then((WAEngine) => {
                let conversation = Promise.promisifyAll(
                    new Conversation({
                        url: WACreds.url,
                        username: WACreds.username,
                        password: WACreds.password,
                        version_date: WACreds.version_date
                    })
                );
                resolve({engine: WAEngine, conversation: conversation});
            }).catch(reject)
    });
};
