 ...
 * @param apiKey
 */
let setupWcs = function (self, wcsUrl, wcsUsername, wcsPassword, versionDate, version, apiKey) {
    try {
        let options;
        if(apiKey) {
            options = {
...
let copyFromNluResponse = function(context, evaluationResponse, NluResponse) {
    if(NluResponse) {
        if(NluResponse.context) {
            ...
            evaluationResponse.response.handleUtterance = NluResponse.context.handleUtterance || evaluationResponse.response.handleUtterance;
            //set routeByEntities flag from NLU context
            evaluationResponse.response.routeByEntities = NluResponse.context.routeByEntities || evaluationResponse.response.routeByEntities;
        }
