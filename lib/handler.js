...
Handler.prototype.initialize = function () {
    ...
    if (this.wcsCredentials) {
        ...
        setupWcs(this, credentials.url, credentials.username, credentials.password, credentials.version_date, credentials.api_key)
    }
};
 ...
 * @param apiKey
 */
let setupWcs = function (self, wcsUrl, wcsUsername, wcsPassword, versionDate, apiKey) {
    try {
...
let copyFromNluResponse = function(context, evaluationResponse, NluResponse) {
    if(NluResponse) {
        if(output.context) {
            ...
            evaluationResponse.handleUtterance = NluResponse.context.handleUtterance || evaluationResponse.response.handleUtterance;
            //set routeByEntities flag from NLU context
            evaluationResponse.routeByEntities = NluResponse.context.routeByEntities || evaluationResponse.response.routeByEntities;
        }
