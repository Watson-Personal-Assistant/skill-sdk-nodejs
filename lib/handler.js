...
Handler.prototype.evaluateRequest = function (request, evaluationResponse, context, callback) {
    getIntent(this, request, (err, intentResult, output) => {
        if (err) {
            ...
            let chosen = sort(bin)[0];
            request.attributes.intent = chosen.intents[0].intent;
...
let filterIntent = function (self, request, intentity) {
    // If no intents or entities
    if (intentity.intents.length === 0 && intentity.entities.length === 0) {
        return false;
    }
    // Intent confidence must be above expertise threshold
    let intentConfidence = intentity.getIntentConfidence();
    let entityConfidence = intentity.getEntityConfidence();
    let threshold = self.manifest.threshold;
    // both intents and entities don't match the threshold
    if (intentConfidence < threshold && entityConfidence < threshold) {
        return false;
    }
    // only the intent doesn't match the threshold - will be deleted
    if(intentConfidence < threshold) {
        intentity.intents = [];
    }
    // only the entity doesn't match the threshold - will be deleted
    if(entityConfidence < threshold) {
        intentity.entities = [];
    }
    else if(intentConfidence < threshold) {
        intentity.addIntent('entities', entityConfidence);
    }
    // Intents filtering
