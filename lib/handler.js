...
const builtInContextUtils = require('wa-built-in-context');
...
Handler.prototype.setBuiltInContextProperty = function(context, path, value) {
    const validationResult = builtInContextUtils.validateBuiltinContext(object);
    if(validationResult.valid) {
        const result = builtInContextUtils.setProperty(context.builtIn, path, value);
        if(result.valid) {
            context.builtIn = validationResult.obj;
            console.info('Successfully set built-in context property ' + path);
            return result
        } else {
            console.error('Error setting built-in context property ' + path);
            return result;
        }
    } else {
        console.error('Error setting built-in context property ' + path);
...
Handler.prototype.deleteBuiltInContextProperty = function(context, path) {
    ...
    if(result.valid) {
        ...
        console.log('Successfully deleted built-in context property ' + path);
        return result;
    } else {
        console.error('Error deleting built-in context property ' + path);
        return result;
    }
};
...
Handler.prototype.getBuiltInContextProperty = function(context, path) {
    ...
    if(result.valid) {
        ...
        console.error('Error getting built-in context property ' + path);
        return result;
    }
};
